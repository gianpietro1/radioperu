class Bulk
  ...
  def self.create_artist
    ...
    if File.exists?(image_file)
      @artist_db = Artist.create(name: @artist_name_id3, image: File.open(image_file, 'r'), bio: @bios_array[@artist_name_id3][:es])    
    else
      default_image_file = File.join(Rails.root, 'app/assets/images', 'default_artist.jpg')
      @artist_db = Artist.create(name: @artist_name_id3, image: File.open(default_image_file, 'r'), bio: @bios_array[@artist_name_id3][:es])          
    end
    I18n.locale = :en
  ...
  def self.create_album
    ...
    if File.exists?(cover_file)
      @album_db = @artist_db.albums.create(name: @album_name_id3, cover: File.open(cover_file, 'r'), year: @song_year_id3)
    else
      default_cover_file = File.join(Rails.root, 'app/assets/images', 'default_album.jpg')
      @album_db = @artist_db.albums.create(name: @album_name_id3, cover: File.open(default_cover_file, 'r'), year: @song_year_id3)
    end
  end
