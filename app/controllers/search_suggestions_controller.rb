class SearchSuggestionsController < ApplicationController
  ...
  def index
    ...
    Song.search_by_name(params[:term]).map do |song|
      @search_array << {label: song.name + " - " + song.album.artist.name, category: t(:songs), model: "Song", model_id: song.id, parent_model_id: song.album.id, gparent_model_id: song.album.artist.id}
    end
